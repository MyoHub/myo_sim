...
|---|---|---|
| Model (human-facing) | `Myo` prefix | `MyoLeg`, `MyoArm` |
| File / registry names | `myo` prefix, lowercase | `myolegs.xml`, `myoarm` |
| Right/left suffix | `_r` / `_l` | `gaslat_r`, `psoas_l` |
| Muscle names | OpenSim naming where possible | `gaslat_r`, `DELT1_r`, `bflh_r` |
| File suffixes | `_assets`, `_chain`, `_tendon`, `_muscle` | `myoleg_r_chain.xml` |
| Attachment sites | `<part>_attach_r` / `<part>_attach_l` | `arm_attach_r` |
| Wrapping sidesiotes | `<WrapName>_site_<muscle>_r` | `AB_at_femshaft_site_addbrev_r` |
| Muscle via-points | `<MuscleName>-P<n>_r` | `addbrev-P1_r` |

---

## What not to do

- Do not declare a site in any file other than the one that owns the site's parent body. If the body lives in `*_chain.xml`, the site must also live there.
- Do not embed cross-part contact pairs inside a part's assets. All inter-part contact pairs go in `myo_sim/models/contacts/`.
- Do not duplicate skeleton bodies across files. Each body is owned by exactly one chain file.
- Do not hand-edit left-side chain files. They are generated by the build pipeline.
- Do not add assistive devices (exoskeletons, orthoses) to body-part asset files. They belong in `myoassist`.
- Do not add `<equality>` constraints that couple joints across parts inside a part's assets — these go in the top-level assembly XML or in the relevant `*_assets.xml` if they are intra-part.
