...
scene without the passive-torso wrapper), use `FRAGMENT_SPEC_BUILDERS`:

```python
from myo_sim import FRAGMENT_SPEC_BUILDERS
spec = FRAGMENT_SPEC_BUILDERS["myohand_r"]()  # returns MjSpec, no torso
```
