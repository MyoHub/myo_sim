...
## 2026-08-02 — Add opt-in series-elastic tendon support
Changed: `myo_sim/build/elastic.py` (new), `tests/test_elastic_tendon.py` (new), `docs/wiki/build-and-composition.md`
Why: MuJoCo's tendons are rigid by default; added an opt-in mechanism to give individual muscles compliant (series-elastic) tendons without changing any shipped model, with the Achilles group (soleus, gastrocnemius medialis/lateralis) as the first validated parameter set and a kinematic self-check every future muscle addition must pass before being trusted.
